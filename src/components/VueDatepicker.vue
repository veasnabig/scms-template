...
<div class="cov-vue-date" :class="option.wrapperClass ? option.wrapperClass : {}">
    <div class="datepickbox">
        <input type="text" title="input date" class="cov-datepicker input-date" readonly="readonly" :placeholder="option.placeholder" v-model="date.time" :required="required" @click="showCheck" @focus="showCheck" :style="option.inputStyle ? option.inputStyle : {}" :class="option.inputClass ? option.inputClass : {}"
        />
    </div>
    <div class="datepicker-overlay" v-if="showInfo.check" @click="dismiss($event)" v-bind:style="{'background' : option.overlayOpacity? 'rgba(0,0,0,'+option.overlayOpacity+')' : 'rgba(0,0,0,0.5)'}">
        <div class="cov-date-body" style="background:#34495e;">
            ...
            <div class="cov-date-monthly">
                ...
                <div class="cov-date-caption" :style="{'color': option.color ? option.color.headerText : '#fff'}">
                    ...
                    <div @click="showMonth" class="month-header">
                        {{displayInfo.month}}
                    </div>
                    <!-- <span @click="showYear"><small>{{checked.year}}</small></span>
                    <br>
                    <span @click="showMonth" class="month-header">{{displayInfo.month}}</span> -->
                </div>
            ...
            <div class="cov-date-box" v-if="showInfo.day">
                <div class="cov-picker-box">
                    <div class="week">
                        <ul>
                            <li v-for="weekie in library.week" class="list-day-week">{{weekie}}</li>
                        </ul>
                    </div>
            ...
            <div class="button-box">
                <span class="btn-footer" @click="showInfo.check=false">{{option.buttons? option.buttons.cancel : 'cancel' }}</span>
                <span class="btn-footer" @click="picked">{{option.buttons? option.buttons.ok : 'ok'}}</span>
            </div>
        </div>
    </div>
</div>
        ...
        option: {
            ...
            default () {
                return {
                    type: 'day',
                    SundayFirst: false,
                    week: ['ច័ន្ទ', 'អង្គា', 'ពុធ', 'ព្រ.ហ', 'សុក្រ', 'សៅរ៍', 'អាទិត្យ'],
