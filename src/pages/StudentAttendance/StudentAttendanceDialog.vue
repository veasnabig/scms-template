<template>
<div>
    <md-dialog :md-active.sync="showStudentAttendanceDialog" :md-click-outside-to-close='false' :md-close-on-esc='false' @keydown.esc="closeDialog">
                ...
                <div class="md-layout-item">
                    <md-table resize v-model="searched" @md-selected="onSelect" md-card :md-selected-value.sync="selected" style="height:30vh;">
                        ...
                        <md-table-row slot="md-table-row" slot-scope="{ item }" :md-disabled="item.name.includes('Stave')" md-selectable="multiple" md-auto-select>
                            ...
                            <md-table-cell md-label="ឈ្មោះសិស្ស">{{ item.name }}</md-table-cell>
...
export default {
    ...
    props: {
        showStudentAttendanceDialog: Boolean,
        cardItem: Object,
        subjectItem: {}
    },
    ...
    methods: {
        ...
        searchOnTable() {
            this.searched = searchByName(this.student, this.search)
        },
        closeDialog() {
            this.selected = [];
            this.date = {
            ...
            this.$emit("closeDialog", !this.showStudentAttendanceDialog);
        },
        onSelect(items) {
            this.selected = items;
        },
